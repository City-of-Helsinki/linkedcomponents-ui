...
import { parseImageServerErrors } from './utils';

const useImageServerErrors = (): UseServerErrorsState => {
  ...
  const showServerErrors = ({
    ...
    if (error instanceof ApolloError) {
      ...
      /* istanbul ignore else */
      if (result) {
        setServerErrorItems(parseImageServerErrors({ result, t }));
...
export default useImageServerErrors;
