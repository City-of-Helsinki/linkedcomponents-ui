...
const keywordSetNames = range(1, KEYWORD_SETS_PAGE_SIZE + 1).map(
  (n) => `Keyword set ${n}`
);
...
const page2KeywordSetNames = range(1, KEYWORD_SETS_PAGE_SIZE + 1).map(
...
const sortedKeywordSetNames = range(1, KEYWORD_SETS_PAGE_SIZE + 1).map(
