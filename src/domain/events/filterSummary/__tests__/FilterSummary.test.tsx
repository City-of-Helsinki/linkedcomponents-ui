...
test('should render and remove text filter', async () => {
  const { history } = renderComponent(`/fi${ROUTES.SEARCH}?full_text=${text}`);
...
test('should remove all filters with clear button', async () => {
  ...
  const { history } = renderComponent(
    `/fi${ROUTES.SEARCH}?full_text=${text}&place=${placeId}&publisher=${publisherId}&end=${end}&start=${start}&type=${type}&eventStatus=${EventStatus.EventCancelled}`
  );
