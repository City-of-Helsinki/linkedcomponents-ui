...
test('should initialize search panel input', async () => {
  ...
  renderComponent(
    `${ROUTES.SEARCH}?full_text=${searchValue}&type=general&eventStatus=EventCancelled`
  );
...
test('should search events with correct search params', async () => {
  ...
  await shouldFilterEventsOrRegistrations({
    ...
    expectedSearch:
      '?eventStatus=EventCancelled&publisher=organization%3A1&type=general&full_text=search',
    history,
